...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://countless-skies-nuxtjs.cdn.prismic.io/api/v2', Object.assign({}, options,  {"routes":[{"type":"page","path":"/:uid"}]}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://countless-skies-nuxtjs.cdn.prismic.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady(() => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=countless-skies-nuxtjs&new=true'
  ...
  if (process.client && process.static && route.path !== '/preview') {
    ...
    const repo = 'countless-skies-nuxtjs'
